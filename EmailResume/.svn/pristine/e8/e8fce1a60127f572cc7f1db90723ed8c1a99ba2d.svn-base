...
public class QianChengParser_2 implements Parser {
    ...
    private static void getResumeInfo(Document doc) throws ParseException { // 从document中抽取出简历信息
        ...
        gender = row[1];
        age = row[2];
        Element tag_id = doc.select("td[style=padding-bottom:18px]").get(0);
        if(tag_id.nextElementSibling().text().contains("ID:")){
            resumeId = tag_id.nextElementSibling().text().substring(3);
        }else{
            resumeId = "";
        }

        if(age.contains("(") && age.contains(")")){
            birthday = age.split("\\(")[1].split("\\)")[0];
            birthdaytimestamp = TransferStringToTimeStamp.transferStringToTimeStamp(birthday);
        }else {
            birthday = Integer.toString(2016 - Integer.parseInt(GetNumFromString(age.split("岁")[0]))) + "." + String.valueOf((int) (Math.random() * 12) + 1) + "." + String.valueOf((int) (Math.random() * 30) + 1);
            birthdaytimestamp = TransferStringToTimeStamp.transferStringToTimeStamp(birthday);
        }
